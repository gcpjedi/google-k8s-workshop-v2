...
1. We don't have an efficient way to update our deployment without downtime. We also can't easily rollback to the previous version if we need to.

Deployments can help to eliminate all these issues. Let's convert our 3 pods to deployments.
...
1. Use `kubectl apply` on all 3 deployments
...
1. Save autoscaler definition as a kubernetes object and examine its content.
    ...
    ```shell
    kubectl get hpa -o yaml > manifests/autoscaler.yml
    ```
